...
public class CustomerView extends JPanel 
{
	...
	class ButtonEventHandler implements ActionListener{
		public void actionPerformed(ActionEvent ev){
			...
			if(o==bCustRegist){  
				registCustomer();  // 회원등록 : 회원 가입 버튼 눌러졌을 경우
			}
	...
	public void registCustomer(){
		...
		// 3. Model 클래스 안에 insertCustomer () 메소드 호출하여 2번 VO 객체를 넘김
			...
			try {
				...
				JOptionPane.showMessageDialog(null, "입력");	// 고객에게 오류 메세지 호출
			} catch (Exception e) {
	...
	public void searchByTel(){
		...
		// 2. Model의 전화번호 검색메소드 selectByTel()  호출
		 try {
			...
			tfCustTel.setText(vo.getPhone());
			tfCustEmail.setText(vo.getEmail());
			tfCustAddr.setText(vo.getAddress());
			tfCustTelAid.setText(vo.getAddPhone());
		} catch (Exception e) {
			e.printStackTrace();
		}
